...
class TestKekRewrap(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.conf.database.connection = 'sqlite:///:memory:'  # Use in-memory SQLite for testing
        ...
        self.crypto_plugin._get_master_key.side_effect = lambda key_type, label: f'{key_type}_{label}_key'
        self.crypto_plugin.pkcs11 = self.pkcs11

        # Mock pkcs11 methods
        self.pkcs11.get_session.return_value = self.session
        self.pkcs11.get_key_handle.side_effect = lambda key_type, label, session: f'{key_type}_{label}_handle'
        self.pkcs11.verify_hmac.return_value = None
        self.pkcs11.unwrap_key.return_value = 'unwrapped_kek'
        self.pkcs11.wrap_key.return_value = {'iv': b'new_iv', 'wrapped_key': b'new_wrapped_key'}
        ...
        with mock.patch('barbican.cmd.pkcs11_kek_rewrap.p11_crypto.P11CryptoPlugin', return_value=self.crypto_plugin):
            with mock.patch.object(utils, 'generate_fullname_for', return_value='plugin_name'):
        ...
        self.rewrapper._session_creator = mock.MagicMock(return_value=self.db_session)
    ...
    def test_rewrap_kek_iv_valid(self):
        ...
        kek.plugin_meta = json.dumps({
            'mkek_label': 'old_mkek_label',
            'hmac_label': 'old_hmac_label',
            'iv': base64.b64encode(b'old_iv').decode(),
            'wrapped_key': base64.b64encode(b'old_wrapped_key').decode(),
            'hmac': base64.b64encode(b'old_hmac').decode()
        })
        kek.id = 'kek_id'
        ...
        # Assertions
        # NEW: Assert verify_hmac was called with correct arguments (iv not None)
        self.pkcs11.verify_hmac.assert_called_once_with(
            'HMAC_old_hmac_label_handle',
            b'old_hmac',
        ...
        self.assertEqual(updated_meta['iv'], base64.b64encode(b'new_iv').decode())
        self.assertEqual(updated_meta['wrapped_key'], base64.b64encode(b'new_wrapped_key').decode())
        self.assertEqual(updated_meta['hmac'], base64.b64encode(b'new_hmac').decode())
    ...
    def test_rewrap_kek_iv_none(self):
        ...
        try:
            ...
            self.fail(
                f"Exception was unexpectedly raised when IV is None, "
                "because Softhsm2(/usr/lib64/pkcs11/libsofthsm2.so) allows key rewrapping when "
                "key_wrap_generate_iv is set to false and key_wrap_mechanism is CKM_AES_KEY_WRAP_PAD.")

        # Assertions: verify_hmac should be called with kek_data = wrapped_key (since iv=None)
        self.pkcs11.verify_hmac.assert_called_once_with(
            'HMAC_old_hmac_label_handle', 
            b'old_hmac', 
        ...
        self.assertEqual(updated_meta['mkek_label'], 'new_mkek_label')
        self.assertEqual(updated_meta['hmac_label'], 'new_hmac_label')
        self.assertEqual(updated_meta['iv'], None) # IV remains None
        self.assertEqual(updated_meta['wrapped_key'], base64.b64encode(b'new_wrapped_key').decode())
        self.assertEqual(updated_meta['hmac'], base64.b64encode(b'new_hmac').decode())
